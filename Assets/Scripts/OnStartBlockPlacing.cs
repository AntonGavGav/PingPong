...
{
    [SerializeField] private Transform topWall, bottomWall, ball, player1, player2, goalBlock1, goalBlock2;

    private void Start()
    {
        //getting usefull info for later
        Camera cam = Camera.main;
        float screenWidth = 1/(cam.WorldToViewportPoint(new Vector3(1,1,0)).x - .5f);
        float screenHeight = 1 / (cam.WorldToViewportPoint(new Vector3(1, 1, 0)).y - .5f);
        ...
        //setting up players 
        player1.localScale = new Vector3(screenWidth / 72, screenHeight / 4f);
        player1.position = new Vector3(-screenWidth / 2.2f, 0);
        player2.localScale = new Vector3(screenWidth / 72, screenHeight / 4f);
